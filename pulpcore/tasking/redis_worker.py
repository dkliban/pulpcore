    ...
    def cleanup_redis_locks_for_worker(self, app_worker):
        ...
        try:
            ...
            task = Task.objects.filter(
                ...
                state__in=TASK_FINAL_STATES
            ).select_related('pulp_domain').first()
            ...
            if task:
                ...
                _logger.warning(
                    ...
                    "(was being executed by missing worker %s)",
                    task.pk,
                    worker_name
                )
            else:
                ...
                for key in self.redis_conn.scan_iter(
                    match=task_lock_pattern, count=100
                ):
                    ...
                    if lock_holder and lock_holder.decode('utf-8') == worker_name:
                        ...
                        try:
                            task = Task.objects.select_related('pulp_domain').get(
                                pk=task_uuid
                            )

                            # Extract resources and release Redis locks
                            exclusive_resources, shared_resources = (
                                extract_task_resources(task)
                            )
                            ...
                            _logger.warning(
                                "Task %s locked by missing worker %s "
            ...
            _logger.error(
                "Error cleaning up locks for worker %s: %s", worker_name, e
            )
