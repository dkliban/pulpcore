    ...
    if execute_now:
        ...
        if are_resources_available(task):
            ...
            if task.state != TASK_STATES.WAITING:
                ...
                _logger.info(
                    "IMMEDIATE DISPATCH: Task %s already in state '%s', releasing locks without execution",
                    task.pk,
                    task.state
                )
                safe_release_task_locks(task)
    ...
    if execute_now:
        ...
        if await async_are_resources_available(task):
            ...
            if task.state != TASK_STATES.WAITING:
                # Task was already executed by a worker, release locks and return
                _logger.info(
                    "IMMEDIATE DISPATCH (async): Task %s already in state '%s', releasing locks without execution",
                    task.pk,
                    task.state
                )
                await async_safe_release_task_locks(task)
