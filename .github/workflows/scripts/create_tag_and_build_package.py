...
helper = textwrap.dedent(
    """\
        ...
        and saved as pulpcore-.tar.gz and pulpcore-[tag]-py3-none-any.whl in the
        'dist' directory.

        If the package does not exist on PyPI, two new packages are built with the names
        pulpcore-[tag].tar.gz and pulpcore-[tag]-py3-none-any.whl. They are
...
package_found = asyncio.run(get_package_from_pypi("pulpcore=={tag.name}", plugin_path))
